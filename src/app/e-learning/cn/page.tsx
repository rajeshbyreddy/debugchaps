              ...
              <div className="text-gray-300 leading-relaxed bg-gray-800 p-6 rounded-lg border-l-4 border-blue-500 shadow-sm">
                {item.answer.split('\n').map((line, i) => {
                  ...
                  const highlightedLine = line
                    .replace(/\b(OSI Model|TCP\/IP|HTTP|HTTPS|TCP|UDP|DNS|DHCP|NAT|ARP|ICMP|VPN|Firewall|CDN|Ethernet|WiFi|IPv4|IPv6|MAC Address|IP Address|Routing|Switching|Load Balancing|Bandwidth|Latency|Throughput|Subnetting)\b/g, '<span class="bg-blue-800 text-blue-100 px-2 py-1 rounded font-semibold">$1</span>')
                    .replace(/\b(\d+(?:\.\d+)?%|\d+ (Mbps|Gbps|ms|GHz))\b/g, '<span class="bg-green-800 text-green-100 px-2 py-1 rounded font-semibold">$1</span>')
